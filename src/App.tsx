...
export default function App() {
  ...
  const [activeTab, setActiveTab] = useState<'units' | 'foundation' | 'kg' | 'quiz' | 'ai' | 'worksheets' | 'achievements'>('kg');
  ...
  const handleSearchQuery = (query: string) => {
    ...
    if (query.includes('حرف') || query.includes('شمسية') || query.includes('قمرية') || query.includes('تأسيس')) {
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 flex flex-col font-sans">
      ...
      <WhatsAppContact phoneNumber="33773659697" displayNumber="+33 7 73 65 96 97" />
      ...
      <footer className="no-print bg-slate-900 text-slate-300 border-t border-slate-800 pt-12 pb-8 px-4 sm:px-6 lg:px-8 mt-auto">
        <div className="max-w-7xl mx-auto">
          {/* WhatsApp Inquiries Banner in Footer */}
          <div className="mb-10 p-6 rounded-3xl bg-gradient-to-r from-emerald-900/80 via-emerald-800/60 to-slate-800 border border-emerald-500/30 flex flex-col md:flex-row items-center justify-between gap-6 shadow-xl">
            <div className="flex items-center gap-4 text-right">
              ...
              <div>
                <h3 className="text-white font-bold text-base sm:text-lg font-alexandria flex items-center gap-2">
                  <span>للتواصل وطلب الدروس الخصوصية والمعلومات</span>
                  <span className="text-xs bg-amber-400 text-slate-950 font-extrabold px-2.5 py-0.5 rounded-full">متاح الآن</span>
                </h3>
                <p className="text-xs sm:text-sm text-slate-300 mt-1">
                  تواصل معنا مباشرة عبر تطبيق الواتساب للاستفسار عن الدروس، حصص التأسيس، وشرح المناهج الدراسية:
                </p>
              </div>
            </div>
            <a
              ...
              className="w-full md:w-auto px-6 py-3.5 bg-emerald-700 hover:bg-emerald-800 active:scale-95 text-white font-bold text-sm rounded-2xl shadow-lg flex items-center justify-center gap-3 transition-all shrink-0 border border-emerald-400/30"
            >
              <MessageCircle className="w-5 h-5 text-white" />
              <span>محادثة واتساب: <span dir="ltr" className="font-mono text-amber-300 font-bold">+33 7 73 65 96 97</span></span>
            </a>
          </div>
